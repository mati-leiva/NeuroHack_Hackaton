...
└── backend/                    ← API + modelo + integración con el juguete
    ...
    ├── detonantes.py               detonantes/estrategias reales por niño/a (CSV 1 y 5 del concurso)
    ├── sensores.py                 procesamiento de señales del juguete (modo personalizado)
    ├── app.py                     API Flask (endpoints del registro, sensor y detonantes)
    ├── bridge_bluetooth.py        puente HC-06 (Arduino) -> API (modo personalizado)
    ├── data/
    │   ├── 1_casos_anonimizados.csv          datos reales del concurso
    │   └── 5_eventos_desregulacion_tutor.csv  datos reales del concurso
    └── arduino/
...
Si eliges un ID `PAC-00x` en el selector (en vez de una familia sintética),
verás además un bloque con detonantes y estrategias **reales**, extraídos
directamente de los datos anonimizados entregados para el concurso — ver
"Detonantes y estrategias reales por niño/a" en `backend/README.md`.
