...
# "desregulaciones_previas" es un CONTEO, no un valor de un solo día: cuenta
# cuántas desregulaciones se registraron en una ventana de los últimos N
# días (no solo "hoy"). Se deja como una constante explícita para que el
# número no quede ambiguo ni en el modelo ni en el formulario del HTML.
# Simplificación de prototipo: en seed_dataset.py cada día se genera de
# forma independiente (Poisson), no como una suma móvil real de N días;
# con datos reales de Bluba convendría calcularlo como una suma rodante
# real sobre esta misma ventana.
...
def calcular_baselines(df_historico: pd.DataFrame, columnas=None, k_shrinkage: int = K_SHRINKAGE):
    """
    Wrapper sobre personalizacion.calcular_baselines. `columnas` es genérico
    a propósito: sirve tanto para las variables del registro diario
    (FEATURES) como para las métricas del juguete (sensores.FEATURES_JUGUETE)
    — misma lógica de personalización para ambas fuentes de datos.
    """
