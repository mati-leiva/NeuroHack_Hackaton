...
Mismo principio aplicado a las señales del dado: un umbral fijo de
"movimiento brusco" no tiene sentido si un niño juega naturalmente más
fuerte que otro. En vez de tener un firmware por niño, **un solo firmware
recibe el umbral calibrado para quien esté jugando en ese momento**.

```
 Arduino (sketch_bluba.ino)              bridge_bluetooth.py                Backend Flask
 ─────────────────────────               ────────────────────               ─────────────
 lee joystick/botón                                                          
...
- `arduino/sketch_bluba.ino`: ya no tiene los umbrales fijos en el código
...
## Detonantes y estrategias reales por niño/a (datos del concurso)

Se recibieron 5 CSV anonimizados para el concurso. De los 5, solo dos tienen
cadencia diaria/temporal útil para un predictor: `4_seguimiento_diario_tutor`
(ya inspiró las variables del modelo) y `5_eventos_desregulacion_tutor`
(eventos de crisis reales, con el detonante exacto y si la estrategia usada
funcionó). Los otros tres (`1_casos_anonimizados`, `2_programas_y_objetivos`,
`3_sesiones_profesionales`) son fichas/gestión de caso, no series de tiempo.

En vez de tocar el modelo predictivo (que sigue entrenado con datos
sintéticos, ver más arriba), se agregó una capa **independiente y opcional**:

- `backend/data/1_casos_anonimizados.csv` y
  `backend/data/5_eventos_desregulacion_tutor.csv` — los datos reales tal
  cual se entregaron (4 casos, 7 eventos).
- `detonantes.py` — `perfil_conocido(usuario_id)` devuelve, si existe,
  el diagnóstico, el perfil sensorial, la lista de eventos documentados
  (detonante + estrategia + si funcionó) y las estrategias que sí lograron
  "Regulación Exitosa". Devuelve `None` si el `usuario_id` no es uno de los
  4 casos reales (p.ej. las familias sintéticas de la demo).
- `GET /api/detonantes/<usuario_id>` — lo expone como endpoint propio.
- También viaja embebido como `known_profile` dentro de la respuesta de
  `POST /api/prediccion`, para que el HTML lo muestre junto a la predicción
  sin una llamada extra.
- En el HTML, el selector de "ID de familia/niño" ahora incluye
  `PAC-001`...`PAC-004` (los casos reales) además de las familias
  sintéticas — al elegir uno de esos, aparece el bloque "Lo que ya sabemos
  de este niño/a" con evidencia real, no una plantilla genérica.

**Limitación honesta**: con solo 7 eventos en 4 niños, esto es un
complemento puntual, no una cobertura general — la mayoría de los niños
(sintéticos o reales futuros) no tendrán todavía este historial. Es la
opción de "esfuerzo bajo/impacto alto" frente a remapear todas las
variables del CSV 4 o reemplazar la etiqueta sintética por una real, que
se dejaron pendientes por lo escaso del dataset entregado.

## Precisión de "desregulaciones_previas"

La variable `desregulaciones_previas` es un **conteo dentro de una ventana
de 3 días** (`VENTANA_DESREGULACIONES_DIAS` en `modelo.py`), no un valor
de "hoy" ni de un período ambiguo. Tanto el formulario del HTML como las
explicaciones que genera el modelo (`"Las desregulaciones en los últimos
3 días..."`) usan ese mismo número explícitamente. Simplificación de
prototipo: en `seed_dataset.py` cada día se genera de forma independiente
(Poisson), no como una suma móvil real de 3 días; con datos reales de
Bluba convendría calcularlo como una suma rodante real sobre esa ventana.
